...
import { getAllProductList } from '../../Service/getAllProductList'
...
export default class AddProductList extends Component {
    ...
    handlePageClick = async (e) => {
        ...
        if (this.state.selectedProduct === "") {
            let result = await getAllProductList(selectedPage);
    ...
    async getProductData() {
        let result = await getAllProductList(this.state.currentPage)
    ...
    allCheckUncheked = (e) => {
        ...
        if (e.target.checked) {
            ...
            this.state.tableData.map((obj, key) => {
                ...
                temp.push(objTemp)
                checkedArray.push(parseInt(obj.productId))
            })
                                        ...
                                        <tr className="Device__table__col">
                                            {(this.state.Role === 'admin' || this.state.Role === 'organization') ? <> <th style={{ display: 'none' }}><div key={key}>
                                                ...
                                                : (this.state.Role === 'user' || this.state.Role === 'empuser') ? <><th style={{ display: 'none' }}><div key={key}>
                                                    ...
                                                    : <>
                                                        <th><div key={key}>
                                                            <input type="checkbox" value={obj.productId} onChange={(e) => this.changeStatus(key, e)} checked={this.state && this.state.checkedStatus[key] && this.state.checkedStatus[key][`${key}`]} />
                                                        </div></th>
                                                    </>}
                                            <td>
