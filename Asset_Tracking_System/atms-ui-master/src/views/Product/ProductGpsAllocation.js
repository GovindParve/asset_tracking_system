...
import { getGpsAssetTagList } from '../../Service/getGpsAssetTagList'
import { getProductListForGpsAlloc } from '../../Service/getProductListForGpsAlloc'
import { getGpsSubProductList } from '../../Service/getGpsSubProductList'
...
import { getAdminwiseGPSTagDropDown } from '../../Service/getAdminwiseGPSTagDropDown'

const SignupSchema = Yup.object().shape({

    productName: Yup.string()
        .required('Select Product Name'),
    allocatedTagName: Yup.string()
        .required('Select Tag Name'),
});
...
export default class ProductGpsAllocation extends Component {
    // interval;
    state = {
    ...
    async componentDidMount() {
        ...
        let resultcategory = await getGpsAssetTagList();
        console.log('result GPS category', resultcategory);
        let resultProduct = await getProductListForGpsAlloc();
        console.log('resultProduct', resultProduct);
        let resultSubProduct = await getGpsSubProductList();
        ...
        resultcategory && resultcategory.data && resultcategory.data.map((obj) => {
            ...
            console.log('Category', tempCategoryObj);
        ...
        this.setState({ allData: tempArray, allCategory: tempCategoryArray, allProduct: tempProductArray, allSubProduct: tempSubProductArray, dispatchList: tempDispatchListArray, allProductDetails: resultSubProduct });

        let resultAdmin = await getGpsAdmin();
    ...
    changeSubPeoductName = (selectedSubProduct) => {
        ...
        this.setState({ selectedSubProduct: selectedSubProduct.value }, async () => {
                ...
                <Formik
                    ...
                    onSubmit={async values => {
                        ...
                        const payload = {
                            ...
                            admin: role === "admin" ? localStorage.getItem("username") : values.admin,
                            fkUserId: fkUserId,
                        ...
                        <Form>
                            <div className="Product_form">
                                ...
                                <div>
                                    ...
                                    <Select options={this.state.allSubProduct} onChange={this.changeSubPeoductName} placeholder="Select Sub-Product Name" />
                                </div>
                                <div>
                                    <label className='label'>Select Admin</label>
                                    <Select options={this.state.allAdminData} placeholder="Select Admin" onChange={async (value) => {
                                        setFieldValue('admin', value.value);
                                        let resultTagList = await getAdminwiseGPSTagDropDown(value.value);
                                        let tempTagArray = resultTagList && resultTagList.data && resultTagList.data.map((obj) => ({
                                            id: `${obj.assetTagId}`,
                                            value: `${obj.assetTagName}`,
                                            label: `${obj.assetTagName}`,
                                            Uniquecode: `${obj.assetUniqueCodeMacId}`,
                                ...
                                <div>
                                    ...
                                    <Select options={this.state.allCategory} onChange={(value) => {
                                        setFieldValue('allocatedTagName', value.value);
                                        this.setState({ selectedCategoryId: value.id, selectedUniqueCode: value.Uniquecode })
                                ...
                                {/* <div>
                                    ...
                                    <Select id="ddlPassport" options={this.state.dispatchList} onChange={this.changeDisptachName} placeholder="Select Dispatch Location" />
