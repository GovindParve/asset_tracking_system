...
import { uploadProductAllocExcel } from '../../Service/uploadProductAllocExcel'


export default class UploadProductAllocExcel extends Component {
    constructor(props) {
        super(props);
        this.state = {
            file: "",
        ...
        this.uploadProductAllocExcel = this.uploadProductAllocExcel.bind(this)
    }
    ...
    async onSubmit(e) {
        ...
        try {
            ...
            let result = await this.uploadProductAllocExcel(this.state.file);
    ...
    async uploadProductAllocExcel(file) {

        const formData = new FormData();

        await formData.append('file', file)
        await formData.append('username', this.state.username)
        return await uploadProductAllocExcel(formData)
            ...
            <div className="user_uploadform" align="center">
                {this.state.loader ? <div>Please wait file is uploading....</div> :
                    <form onSubmit={this.onSubmit}>
                        <h3> Upload Product-Tag Allocation Excel</h3>
                        <br />
