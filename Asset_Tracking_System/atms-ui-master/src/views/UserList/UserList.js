...
export default class UserList extends Component {
  ...
  handlePageClick = async (e) => {
    ...
    let result = await getPageWiseUser(
      selectedPage,
    ...
    if (this.state.selectedOrg !== "") {
      ...
      axios
        .get(
          `/user/get-all-userNameWise-list/${this.state.currentPage}?userName=${this.state.selectedOrg}&role=organization&searchrole=&category=${category}`,
          {
      ...
      axios
        .get(
          `/user/get-all-userNameWise-list/${this.state.currentPage}?userName=${this.state.selectedAdmin}&role=admin&searchrole=&category=${category}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        )
  ...
  async componentDidMount() {
    try {
      setTimeout(async () => {
        this.setState({ loader: true, Role: localStorage.getItem("role"), organization: localStorage.getItem("organization") });
        this.getData();
  ...
  downloadUserExcel = async () => {
    ...
    axios
      .get(
        `/user/report/get-User-report-export-excel-or-pdf-download?fileType=${fileType}&fkUserId=${fkUserId}&role=${role}&category=${category}`,
        {
          headers: { Authorization: `Bearer ${token}` },
  ...
  changeOrgName = async (selectOrg) => {
    ...
    axios
      .get(
        `user/get-adminlist_for_dropdown?fkUserId=${selectOrg.id}&role=organization&category=BLE`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      )
