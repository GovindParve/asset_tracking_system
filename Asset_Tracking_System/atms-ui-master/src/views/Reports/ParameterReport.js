...
export default class ParameterReport extends Component {
  constructor(props) {
    super(props);
    this.state = {
      offset: 0,
  ...
  async componentDidMount() {
    try {
      // let result = await getDynamicColumnList();
      // console.log("getDynamicColumn", result.data.list)
      // result.data.list.map((obj) => {
      ...
      console.log('Column Drowpdown List', columnList.data)
      let tempArray = []
  ...
  render() {
    const { tableColumnData, heading, allColumn, row,loader } = this.state
