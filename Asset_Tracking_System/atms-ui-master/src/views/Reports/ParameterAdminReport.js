...
export default class ParameterAdminReport extends Component {
  constructor(props) {
    super(props);
    this.state = {
      offset: 0,
  ...
  changeColumn = (selectedOptions) => {
    ...
    this.setState({ selectedColumn: selectedOptions.value }, async () => {
      ...
      if (this.state.Role === 'admin' || this.state.Role === "organization") {
        let resultAllData = await getAdminDrowpdownList();
        let tempTagArray = [
          "Sr_No",
          "Asset_Tag_Name",
          "Date",
  ...
  downloadFile = async () => {
    ...
    axios.get(`/asset/report/get-tracking-report-for-dynamic-columns?columnname=${this.state.selectedColumn}&fileType=${fileType}&category=${category}&fkUserId=${fkUserId}&role=${role}`, {
      ...
      swal("Sorry", "Please select column", "warning");
      this.setState({reportload:false})
    }
  ...
  render() {
    const { tableColumnData, heading, row, loader } = this.state
