...
export default class UserAdd extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      let resultGpsOrg = await getGpsOrgWiseList();
      console.log("result", resultGpsOrg);
      let tempGpsOrgArray = [];
      if (resultGpsOrg && resultGpsOrg.data && resultGpsOrg.data.length != 0) {
        resultGpsOrg.data.map((obj) => {
          let tempObj = {
            id: `${obj.pkuserId}`,
            value: `${obj.username}`,
            label: `${obj.username}`,
          };
          tempGpsOrgArray.push(tempObj);
        });
      }
      // else {
      //   swal(
      ...
      this.setState({ allGpsOrgList: tempGpsOrgArray }, () => {
        console.log("allGpsOrgList List", this.state.allGpsOrgList);
      });
      ...
      let resultGpsAdmin = await getAdminWiseGPSUserList();
                        ...
                        {this.state.optionAllUser === "user" &&
                          ...
                          <>
                            <div>
                              ...
                              <Select
                                options={this.state.allGpsOrgList}
                                onChange={this.changeGpsOrgName}
