...
export default class UserTrackingList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      offset: 0,
  ...
  async componentDidMount() {
    try {
      ...
      console.log("User Column Data", resultColumn.data.list)
      resultColumn.data.list.map((obj) => {
        console.log('user Column', obj);
  ...
  async receivedData() {
    setTimeout(async () => {
      ...
      } else if (resultPage && resultPage.data && resultPage.data.length === "") {
        swal("Sorry", "Data is not present", "warning");
      }
    }, 200)
  }
