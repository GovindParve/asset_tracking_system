...
export default class AdminTrackingList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      offset: 0,
  ...
  async componentDidMount() {
    try {
      ...
      let [resultAssetTagList, resultAssetGatewayList, setRole] = await Promise.all([listAssetTagforTrack(), localStorage.getItem('role')]);
      let tempTagArray = []
      ...
      this.setState({ tagList: tempTagArray, Role: setRole, loader: false }, () => {
        console.log('Admin Tag List', this.state.tagList)
      })
      ...
      if (resultColumn && resultColumn.data.list && resultColumn.data.list.length !== 0) {
        this.setState({ allColumn: resultColumn.data.list }, () => {
          console.log('Admin Column', this.state.allColumn)
        })
      } else {
        swal("Sorry", "Dynamic Admin Column Data is not present", "warning");
      }
  ...
  async receivedData() {
    setTimeout(async () => {
      ...
      console.log("Admin Tracking List", resultPage.data)
    ...
    if (selectedOptions.value == "Select device") {
      ...
      this.setState({ selectedDevice: selectedOptions.value },
        async () => {
          ...
          console.log("Admin Viewresult", resultTag)
